...
const LoginPage: React.FC<LoginPageProps> = ({ onSwitchToSignup, onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const { token } = await dataService.login(email, password);
      localStorage.setItem('authToken', token);
      onLogin(); // App.tsx will re-check auth state
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          ...
          <button onClick={onSwitchToSignup} className="font-medium text-primary-600 hover:text-primary-500 transition">
            Sign up for free
          </button>
        </p>
      </div>
